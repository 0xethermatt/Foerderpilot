...
const URL_ERROR_MESSAGES: Record<string, string> = {
  link_expired: 'Dieser Link ist abgelaufen oder wurde bereits verwendet. Bitte fordern Sie einen neuen Link an.',
...
export default function LoginForm({ urlError }: { urlError?: string }) {
  ...
  const errorMessage =
    state?.error ??
    (urlError ? (URL_ERROR_MESSAGES[urlError] ?? 'Anmeldung fehlgeschlagen. Bitte versuchen Sie es erneut.') : undefined);
  ...
  if (state?.sent) {
    return (
      <div className="w-full max-w-sm space-y-4 text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">E-Mail gesendet</h2>
  ...
  return (
    <div className="w-full max-w-sm space-y-6">
      <div className="text-center">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Anmelden mit Magic Link</p>
      </div>

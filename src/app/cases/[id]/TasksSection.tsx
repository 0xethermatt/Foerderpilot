...
function NewTaskForm({ caseId, onSuccess }: { caseId: string; onSuccess: () => void }) {
  ...
  return (
    <form action={formAction} className="mt-4 space-y-3 border-t border-gray-100 dark:border-gray-800 pt-4">
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-400 resize-none placeholder:text-gray-400 dark:placeholder:text-gray-500"
        />
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Fällig am</label>
          <input
            name="due_date"
            type="date"
            className="w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-400"
          />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Priorität</label>
          <select
            name="priority"
            defaultValue="normal"
            className="w-full rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-400"
...
function TaskItem({ task }: { task: TaskRow }) {
  ...
  return (
    <div className={`flex items-start gap-3 py-2.5 border-b border-gray-50 dark:border-gray-800 last:border-0 ${task.completed ? 'opacity-50' : ''}`}>
      {/* Priority dot */}
      <span
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium text-gray-900 dark:text-gray-100 ${task.completed ? 'line-through' : ''}`}>
          {task.title}
        </p>
        {task.description && (
    ...
    <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-5">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Aufgaben</h2>
          {open.length > 0 && (
      ...
      {open.length === 0 && !showForm && (
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-3">Keine offenen Aufgaben.</p>
      )}
      {open.map((t) => (

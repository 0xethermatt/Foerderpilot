...
const ASSESSMENT_CONFIG = {
  likely_eligible: { label: 'Wahrscheinlich förderfähig', cls: 'bg-green-100 text-green-800' },
  unclear:         { label: 'Unklar',                      cls: 'bg-yellow-100 text-yellow-800' },
  critical:        { label: 'Kritisch',                    cls: 'bg-red-100 text-red-800' },
} as const;

const RISK_CONFIG = {
  green:  { label: 'Risiko: Niedrig',   cls: 'bg-green-50 text-green-700' },
  yellow: { label: 'Risiko: Mittel',    cls: 'bg-yellow-50 text-yellow-700' },
  red:    { label: 'Risiko: Hoch',      cls: 'bg-red-50 text-red-700' },
} as const;
...
const REVIEW_CONFIG = {
  pending:  { label: 'Prüfung offen',  cls: 'bg-yellow-50 text-yellow-700' },
  approved: { label: 'Freigegeben',    cls: 'bg-green-50 text-green-700' },
  rejected: { label: 'Abgelehnt',      cls: 'bg-red-50 text-red-700' },
} as const;
...
function RunButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="w-full rounded-md border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 disabled:opacity-50 transition-colors flex items-center justify-center gap-1.5"
    >
  ...
  return (
    <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
      <form action={markAICheckApprovedAction} className="flex-1">
        ...
        <button
          type="submit"
          className="w-full rounded-md border border-green-200 bg-green-50 px-3 py-1.5 text-xs font-medium text-green-700 hover:bg-green-100 transition-colors flex items-center justify-center gap-1"
        >
      ...
      <form action={markAICheckRejectedAction} className="flex-1">
        ...
        <button
          type="submit"
          className="w-full rounded-md border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-medium text-red-700 hover:bg-red-100 transition-colors flex items-center justify-center gap-1"
        >
...
function AICheckCard({
  ...
  return (
    <div className="border border-gray-200 rounded-md overflow-hidden">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-start justify-between gap-2 px-3 py-2.5 text-left bg-gray-50 hover:bg-gray-100 transition-colors"
      >
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {check.confidence && (
              <span className="text-xs text-gray-500">{CONFIDENCE_LABEL[check.confidence]}</span>
            )}
        ...
        <div className="px-3 pb-3 pt-2 space-y-3 text-sm">
          ...
          {check.status === 'failed' && (
            <div className="rounded-md bg-red-50 border border-red-200 px-3 py-2">
              <p className="text-xs font-medium text-red-700">KI-Analyse fehlgeschlagen</p>
              {result === null && (
                <p className="text-xs text-red-600 mt-0.5">Kein gültiges Ergebnis gespeichert.</p>
              )}
            </div>
          )}
          ...
          {result && (
            <>
              {/* Summary */}
              {result.summary_de && (
                <p className="text-sm text-gray-800">{result.summary_de}</p>
              )}
                  ...
                  <ul className="space-y-0.5">
                    {result.missing_information.map((item, i) => (
                      <li key={i} className="flex items-start gap-1.5 text-xs text-gray-700">
                        <span className="mt-1 flex-shrink-0 h-1.5 w-1.5 rounded-full bg-gray-400" />
                        {item}
                      </li>
                    ))}
                  </ul>
                    ...
                    {result.possible_bonuses.map((b, i) => {
                      ...
                      return (
                        <li key={i} className="text-xs">
                          ...
                          <span className="text-gray-500"> · {b.reason_de}</span>
                        </li>
                      );
                    })}
                  </ul>
                    ...
                    {result.detected_risks.map((r, i) => (
                      <li key={i} className="flex items-start gap-2">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs text-gray-800">{r.risk_de}</p>
                          <p className="text-xs text-gray-500 italic mt-0.5">{r.recommended_action_de}</p>
                        </div>
                      </li>
                    ))}
                  </ul>
              ...
              {result.customer_message_draft_de && (
                <div>
                  ...
                  {emailOpen && (
                    <pre className="mt-1.5 text-xs text-gray-700 bg-gray-50 border border-gray-200 rounded p-2.5 whitespace-pre-wrap font-sans leading-relaxed">
                      {result.customer_message_draft_de}
                    </pre>
                  )}
                </div>
                  ...
                  <ul className="space-y-0.5">
                    {result.internal_notes_de.map((note, i) => (
                      <li key={i} className="flex items-start gap-1.5 text-xs text-gray-600">
                        <Info className="mt-0.5 flex-shrink-0 h-3 w-3 text-gray-400" />
                        {note}
                      </li>
                    ))}
                  </ul>
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-5">
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-sm font-semibold text-gray-900">KI-Fördercheck</h2>
        {checks.length > 0 && (
          <span className="text-xs text-gray-400">{checks.length}</span>
        )}
      </div>

      {/* Disclaimer */}
      <p className="text-xs text-amber-700 bg-amber-50 rounded-md px-2.5 py-1.5 mt-2">
      ...
      {/* Checks list */}
      {checks.length === 0 ? (
        <p className="text-xs text-gray-400 mt-3">Noch kein KI-Check durchgeführt.</p>
      ) : (
